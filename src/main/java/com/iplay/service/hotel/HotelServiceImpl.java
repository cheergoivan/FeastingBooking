	...
	public List<SimplifiedHotelDTO> listHotel(int pageNumber, int pageSize) {
		...
		List<SimplifiedHotelDTO> hotels = hotelDOs.stream().map(hotelDO -> {
			return new SimplifiedHotelDTO(hotelDO.getId(), hotelDO.getName(),
					...
					new int[]{hotelDO.getMinimumTables()==Integer.MAX_VALUE?0:hotelDO.getMinimumTables(), hotelDO.getMaximunTables()==-1?0: hotelDO.getMaximunTables()},
					hotelDO.getAddress().getDistrict(), 0, hotelDO.getPicturesAsArray().length>0?ResourcesUriBuilder.buildUri(hotelDO.getPicturesAsArray()[0]):"");
